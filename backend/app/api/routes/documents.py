...
from app.core.auth import auth_client, require_role
...
# Any logged-in user can view their own documents
...
# Only employee/manager/admin roles can upload — adjust as needed
@documents_router.post("/upload")
async def upload_document(
    file: UploadFile = File(),
    auth_session=Depends(require_role("admin", "manager", "employee")),
) -> DocumentWithoutContent:
    user = auth_session.get("user")
...
# Any logged-in user can fetch content (FGA governs actual document-level access)
@documents_router.get(
    "/{document_id}/content",
...
# Only managers/admins can share documents
...
async def delete_document(
    document_id: str, auth_session=Depends(require_role("admin"))
):
    with Session(engine) as db_session:
        shared_with = db_session.exec(
            select(Document.shared_with).where(col(Document.id) == document_id)
