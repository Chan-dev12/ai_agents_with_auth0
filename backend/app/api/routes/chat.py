...
#             content.setdefault("config", {}).setdefault("configurable", {})["_credentials"] = {}
...
async def api_route(
    request: Request,
    full_path: str,
    auth_session=Depends(auth_client.require_session),  # NEW: require login
):
    try:
        ...
        if request.method in ("POST", "PUT", "PATCH") and body:
            ...
            configurable["user_roles"] = user_roles  # NEW: pass roles into the graph
            configurable["user_email"] = user.get("email")  # useful for logging/audit
            body = json.dumps(content).encode("utf-8")
